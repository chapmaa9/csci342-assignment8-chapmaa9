...
app.use(cors({
    origin: [
        "http://localhost:5173",
        "https://csci342-assignment8-varcel-chapmaa9.vercel.app"
    ],
    credentials: true,
  }));
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      token: token,   // the JWT — the client saves this in localStorage
    });
  } catch (error) {
...
app.post("/api/logout", (req, res) => {
  ...
  try {
    jwt.verify(authArr[1], process.env.JWT_SECRET);
  } catch {}
